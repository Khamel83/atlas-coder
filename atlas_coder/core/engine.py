...
class AtlasCoderEngine:
    ...
    def _load_cost_tracker(self, daily_budget: float) -> CostTracker:
        ...
        return CostTracker(daily_budget=daily_budget, last_reset=datetime.now().date().isoformat())
    ...
    def _configure_dspy(self) -> None:
        ...
        dspy.settings.configure(
            cache_turn_on=True,
            cache_db_path=str(self.cache_dir / "dspy_cache.db")
        )
    ...
    def _setup_model(self) -> dspy.LM:
        ...
        if model_config.provider == "openrouter":
            ...
            if not api_key:
                raise ValueError("OPENROUTER_API_KEY environment variable required for cloud models")
    ...
    def estimate_cost(self, input_tokens: int, output_tokens: int = 1000) -> float:
        ...
        strategy_key = "quality-optimal" if self.model_strategy == "quality-optimal" else "cost-optimal"
        model_config = self.DEFAULT_MODELS[strategy_key]
        
        total_tokens = input_tokens + output_tokens
        return (total_tokens / 1_000_000) * model_config.cost_per_million_tokens
    
    def can_execute(self, estimated_input_tokens: int = 2000, estimated_output_tokens: int = 1000) -> bool:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "remaining_budget": self.cost_tracker.daily_budget - self.cost_tracker.current_cost,
    ...
    def execute_with_tracking(self, dspy_module, **kwargs) -> Any:
        ...
        if not self.can_execute():
            raise RuntimeError(f"Daily budget of ${self.cost_tracker.daily_budget} exceeded. Current cost: ${self.cost_tracker.current_cost:.4f}")
...
def get_engine(model_strategy: str = "cost-optimal", daily_budget: float = 1.0) -> AtlasCoderEngine:
