...
class WorkflowOrchestrator:
    ...
    def __init__(
        ...
        self.engine = AtlasCoderEngine(
            model_strategy=model_strategy, daily_budget=daily_budget
        )
...
""",
    ...
    def _save_result(self, result: dict[str, Any], filename: str) -> None:
        ...
        if not self.yolo_mode:
            save = (
                console.input(f"Save result to {filename}? (y/N): ")
                .lower()
                .startswith("y")
            )
            if not save:
    ...
    def generate(
        ...
        try:
            ...
            result = self.engine.execute_with_tracking(
                self.code_generator, requirements=requirements, language="Python"
            )
            ...
            console.print(
                Panel(
                    f"[bold green]Generated Code:[/bold green]\n{result.code}",
            ...
            self._save_result(
                output,
                f"atlas_output/generated_code_{len(requirements.split())}_words.json",
            )
    ...
    def fix_bug(
        self, code_file: str, error_message: str = "", level: str = "detailed"
    ) -> str:
...
""",
            ...
            console.print(
                Panel(
                    f"[bold green]Fixed Code:[/bold green]\n{result['fix']['fixed_code']}",
                    title="Fixed Code",
                    border_style="green",
                )
            )
            ...
            if result.get("tests"):
                console.print(
                    Panel(
                        f"[bold purple]Test Code:[/bold purple]\n{result['tests']['test_code']}",
            ...
            # Save fixed code
            if self.yolo_mode or console.input(
                f"Apply fix to {code_file}? (y/N): "
            ).lower().startswith("y"):
                # Backup original
                ...
                console.print(
                    f"[green]✓ Fix applied to {code_file} (backup saved as {backup_path})[/green]"
                )

            # Save full result
            self._save_result(
                result, f"atlas_output/bug_fix_{Path(code_file).stem}.json"
            )
    ...
    def analyze(self, target: str, level: str = "detailed") -> str:
        ...
        try:
            ...
            result = self.engine.execute_with_tracking(
                self.code_analyzer, code=code, focus="all"
            )
...
""",
    ...
    def refactor(self, target: str, level: str = "detailed") -> str:
        ...
        try:
            ...
            console.print(
                Panel(
                    f"[bold green]Refactored Code:[/bold green]\n{result.refactored_code}",
            ...
            if self.yolo_mode or console.input(
                f"Apply refactoring to {target}? (y/N): "
            ).lower().startswith("y"):
                # Backup original
                ...
                console.print(
                    f"[green]✓ Refactoring applied to {target} (backup saved as {backup_path})[/green]"
                )
    ...
    def project(self, description: str, level: str = "premium") -> str:
        ...
        try:
            ...
            if result["generated_code"]:
                console.print(
                    Panel(
                        f"[bold green]Generated Project Code:[/bold green]\n{result['generated_code']['code']}",
                        title="Project Generation",
                        border_style="green",
                    )
                )
                console.print(
                    f"[blue]Structure:[/blue] {result['generated_code']['structure']}"
                )
                console.print(
                    f"[blue]Usage:[/blue] {result['generated_code']['usage_example']}"
                )

            # Save project structure
            self._save_result(
                result,
                f"atlas_output/project_{description.replace(' ', '_')[:20]}.json",
            )
