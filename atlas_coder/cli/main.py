...
@click.option('--model-strategy', default='cost-optimal', 
...
def cli(ctx, yolo, model_strategy, budget):
    ...
    if model_strategy != 'local-only' and not os.getenv("OPENROUTER_API_KEY"):
        console.print("[bold red]Error:[/bold red] OPENROUTER_API_KEY environment variable required for cloud models")
        console.print("[yellow]Set OPENROUTER_API_KEY in your environment or use --model-strategy local-only[/yellow]")
        sys.exit(1)
...
@click.option('--level', default='detailed', 
...
@click.option('--level', default='premium', 
              type=click.Choice(['quick', 'detailed', 'premium']), 
              help='Execution level.')
@click.pass_context
...
""", title="Cost Analysis", border_style="green" if status['can_make_calls'] else "red"))
